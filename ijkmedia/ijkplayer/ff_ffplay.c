    ...
    if (ffp->pf_playback_rate <= 1.0) {
        ...
        if (ffp->show_status) {
            ...
            if (!last_time || (cur_time - last_time) >= 30000) {
                ...
                ALOGE("%7.2f %s:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB f=%"
                PRId64
                "/%"
                PRId64
                "   \r",
        ...
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    av_log(NULL, AV_LOG_INFO, "video accurate_seek start, is->seek_pos=%"
                    PRId64
                ...
                av_log(NULL, AV_LOG_INFO,
                       "video accurate_seek is ok, is->drop_vframe_count =%d, is->seek_pos=%"
                PRId64
                ...
                if (!isnan(frame_pts)) {
                    ...
                    if ((audio_clock * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION) {
                        if (is->drop_aframe_count == 0) {
                            ...
                            SDL_UnlockMutex(is->accurate_seek_mutex);
                            av_log(NULL, AV_LOG_INFO, "audio accurate_seek start, is->seek_pos=%"
                            PRId64
