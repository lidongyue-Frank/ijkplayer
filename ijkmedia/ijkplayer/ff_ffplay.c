    ...
    if (ffp->pf_playback_rate <= 1.0)
    {
                ...
                if (is->subtitle_st)
                {
                    while (frame_queue_nb_remaining(&is->subpq) > 0)
                    {
                        ...
                        if (sp->serial != is->subtitleq.serial ||
                            (is->vidclk.pts > (sp->pts + ((float)sp->sub.end_display_time / 1000))) || (sp2 && is->vidclk.pts > (sp2->pts + ((float)sp2->sub.start_display_time / 1000))))
                        {
                            if (sp->uploaded)
                            {
        ...
        if (ffp->show_status)
        {
            ...
            if (!last_time || (cur_time - last_time) >= 30000)
            {
                ...
                ALOGE("%7.2f %s:%7.3f fd=%4d aq=%5dKB vq=%5dKB sq=%5dB f=%" PRId64
                      "/%" PRId64
                      "   \r",
                      get_master_clock(is),
    ...
    else
    {
        ...
        retry2:
            ...
            else
            {
                ...
                delay = compute_target_delay(ffp, last_duration, is);
                // delay=0;
                if (ffp->pf_playback_rate <= 16.0)
                {
                    // ALOGE("倍数 pf_playback_rate 4.0 ");
        ...
        if (!isnan(pts))
        {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) || deviation > MAX_DEVIATION)
            {
                ...
                else
                {
                    av_log(NULL, AV_LOG_WARNING,
                           "video accurate_seek is error, is->drop_vframe_count=%d, now = %" PRId64
                           ", pts = %lf\n",
            ...
            else
            {
                av_log(NULL, AV_LOG_INFO,
                       "video accurate_seek is ok, is->drop_vframe_count =%d, is->seek_pos=%" PRId64
                       ", pts=%lf\n",
    ...
    do
    {
        ...
        if (got_frame)
        {
            ...
            if (ffp->enable_accurate_seek && is->audio_accurate_seek_req && !is->seek_req)
            {
                ...
                {
                    av_log(NULL, AV_LOG_INFO,
                           "audio accurate_seek is error, is->drop_aframe_count=%d, now = %" PRId64
                           ", audio_clock = %lf\n",
...
#if CONFIG_AVFILTER
        if (last_w != frame->width || last_h != frame->height || last_format != frame->format || last_serial != is->viddec.pkt_serial || ffp->vf_changed || last_vfilter_idx != is->vfilter_idx)
        {
            SDL_LockMutex(ffp->vf_mutex);
            ffp->vf_changed = 0;
            av_log(NULL, AV_LOG_DEBUG,
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    {
        set_clock_at(&is->audclk, is->audio_clock - (double)(is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec - SDL_AoutGetLatencySeconds(ffp->aout), is->audio_clock_serial,
...
//      av_dict_set(&ffp->format_opts,"max_analyze_duration","5 * AV_TIME_BASE", 0);//设置超时5秒
    ...
    for (;;)
    {
        ...
        if (ffp->infinite_buffer < 1 && !is->seek_req &&
            ...
            (is->audioq.size + is->videoq.size + is->subtitleq.size > ffp->dcc.max_buffer_size
             ...
             || (stream_has_enough_packets(is->audio_st, is->audio_stream, &is->audioq, MIN_FRAMES) &&
                 stream_has_enough_packets(is->video_st, is->video_stream, &is->videoq, MIN_FRAMES) && stream_has_enough_packets(is->subtitle_st, is->subtitle_stream, &is->subtitleq, MIN_FRAMES))))
        {
            if (!is->eof)
            {
        ...
        if (ffp->is_record)
        {
            ...
            {
                // 可以录制时，写入文件
                if (0 != ffp_record_file2(ffp, pkt))
                {
                    ffp->record_error = 1;
                    ffp_stop_record2(ffp);
                }
            }
        }
        // 结束新加
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename, AVInputFormat *iformat)
{
    ...
    if (ffp->async_init_decoder && !ffp->video_disable && ffp->video_mime_type &&
        strlen(ffp->video_mime_type) > 0 && ffp->mediacodec_default_name && strlen(ffp->mediacodec_default_name) > 0)
    {
...
static void ffp_log_callback_brief(void *ptr, int level, const char *fmt, va_list vl)
{
    ...
    int ffplv
        av_unused = log_level_av_to_ijk(level);
...
int ffp_stop_l(FFPlayer *ffp)
{
    ...
    if (ffp->enable_accurate_seek && is && is->accurate_seek_mutex && is->audio_accurate_seek_cond && is->video_accurate_seek_cond)
    {
        SDL_LockMutex(is->accurate_seek_mutex);
        is->audio_accurate_seek_req = 0;
        is->video_accurate_seek_req = 0;
    ...
    if (ffp->is_record)
    {
        ...
        if (packet != NULL && packet->size > 0 && packet->data != NULL)
        {
            if (0 == av_packet_ref(pkt, packet))
            {
                ...
                // av_log(ffp, AV_LOG_INFO, " pkg->stream_id:%d", pkt->stream_id);
                av_log(ffp, AV_LOG_INFO, " pkg->stream_index:%d", pkt->stream_index);

                //av_log(ffp, AV_LOG_INFO, "   pkg->codec_id:%d", pkt->codec_id);

                //av_log(ffp, AV_LOG_INFO, " ffp->data :%d %d %d %d \n", pkt->data[0], pkt->data[1], pkt->data[2], pkt->data[3]);
...
int ffp_start_record2(FFPlayer *ffp, const char *file_name)
{
    ...
    VideoState *is = ffp->is;
    av_log(ffp, AV_LOG_ERROR, "ffp_start_record2 111111111111111111111111111");
    ffp->m_ofmt_ctx = NULL;
    ffp->m_ofmt = NULL;
    ffp->is_record = false;
    ffp->record_error = 0;
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        // 将输入视频/音频的参数拷贝至输出视频/音频的AVCodecContext结构体
        av_log(ffp, AV_LOG_DEBUG, "in_stream->codec_type；%d\n", in_stream->codec->codec_type);
        av_log(ffp, AV_LOG_DEBUG, "in_stream->codec_id；%d\n", in_stream->codec->codec_id);
...
//        if (ffp->m_ofmt_ctx->oformat->flags & AVFMT_GLOBALHEADER) {
//            out_stream->codec->flags |= AV_CODEC_CTX_FLAG_GLOBAL_HEADER;
//        }
    }
    av_dump_format(ffp->m_ofmt_ctx, 0, file_name, 1);
